...
| UI/UX pass (2026-06-11, late) | Three-agent audit (inventory, nav/IA, visual consistency) then implemented with owner sign-off: CV sub-nav tab bar replacing the buried "More" dropdown + footnote-only Tags path; /cv actions 4→2 (Share + merged Download popover = print-to-PDF + column-picker export; CvExportButton→CvDownloadButton); Dashboard renamed "Imports" with calm summary default (URL table behind ?filter=, dupes removed, Export-errors→/admin, AutoScanOnVisit on /cv only); conditional Admin NavLink (isAdminEmail in lib/admin.ts); app-wide design-token migration (new text-ui 14px token, Button/Badge/StatusPill/popovers/settings/verify/onboarding/admin off raw px + shadcn family, secondary Button variant = deprecated alias of outline, admin pages aligned to max-w-5xl). | `components/CvSubNav.tsx`, `CvDownloadButton.tsx`, `app/(app)/dashboard/page.tsx`, `tailwind.config.ts` |
| Design revamp (2026-06-11, after) | Landing rebuilt to lead with the artifact: hero copy "Your debate history, readable and ready to share."; primary CTA "Build my debate CV" (was "Sign in with Google"); sample CV right of the headline; "Privacy" replaces "Colophon" as a labelled grid of operational facts; "Questions" replaces "Letters". Signed-in IA promoted Growth (was the buried Analytics sub-tab) and Share (was at /settings/sharing) to top-level alongside CV / Imports / Settings; new `components/AppNav.tsx` resolves the active item by longest-prefix-wins. Page mastheads moved off italic Fraunces (`font-serif text-hN italic`) to upright Space Grotesk (`font-display text-hN font-semibold`); /cv kicker is now "PRIVATE RECORD · UPDATED …" (was "DEBATE CV — VOL. III · COMPILED …"). Button `primary` variant points at the `--primary` token (tournament green) rather than `bg-ink`. Brand assets (favicon, apple-touch, OG image) rebuilt to match. Single source of truth: `docs/DESIGN_INSTRUCTIONS.md`. | `app/page.tsx`, `components/AppNav.tsx`, `components/BrandMark.tsx`, `components/ui/Button.tsx`, `app/opengraph-image.tsx`, `docs/DESIGN_INSTRUCTIONS.md` |
...
5. Minor UI advisories (dead `mono`/`accent` props, Failed-tile hint when count is 0, `enqueueUrl` doesn't reset `abandoned → pending`).
...
- **Design language: see `docs/DESIGN_INSTRUCTIONS.md`** — single source of truth for visual, IA, and copy decisions. Treat it as a hard contract: lead with the artifact (the CV) not the importer, "Build my debate CV" not "Sign in with Google", privacy stated concretely and operationally. Editorial-magazine framing (Vol./Editor's Note/Colophon/decorative italic) is explicitly retired (brief §11). Implementation notes that remain factual: HSL CSS variables in `app/globals.css` carry the palette; `data-theme` on `<html>` (no-FOUC inline script in `app/layout.tsx`, `ThemeToggle` in the app header, localStorage + OS fallback) selects light vs dark; public CV (`/u/<slug>`) and print are FORCED light because credential artifacts must look identical for every viewer; type stack is Space Grotesk display / Inter body / IBM Plex Mono numerals (`.num` / `font-mono`); token names (`ink`/`paper`/`oxblood`/`primary`) are the abstract fg/bg/accent contract used across ~40 files — re-point values, don't rename.
