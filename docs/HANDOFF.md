...
- Design language: **Tab Sheet** (owner-ruled teardown 2026-06-11, late — supersedes both the editorial theme AND the same-day Tab Room Terminal retheme, which died under the design doc's "never a hacker dashboard" rule). Spec + owner rulings D1–D5: `docs/DESIGN_TEARDOWN.md`; constitution: `docs/DESIGN_INSTRUCTIONS.md`. Single sheet-white theme (dark mode DELETED — tokens, ThemeToggle, no-FOUC script all gone); Archivo display (wdth axis, `.display-expanded`), Libre Franklin body, Spline Sans Mono data (tabular-nums global on mono); editorial aliases executed (ink/paper/oxblood/kicker/byline → record-*/sheet/eyebrow/meta; no font-serif slot); record primitives in components/ui (SectionHeader/.tab-header, StatBlock, SourceBadge, BreakMarker, ResultLine); gold = earned results only, green = act/verified only; per-CV og:image share card at app/u/[slug]/opengraph-image.tsx. Teardown chunks 1–9 shipped; remaining: deeper onboarding/settings re-skin, a11y/mobile QA sweep (chunk 10), and the single "gold settle" ingest animation (chunk 11 — motion last).
