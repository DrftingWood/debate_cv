    ...
    it('outround round labels classify to a canonical stage', () => {
      ...
      const outroundish = [...roundLabels].filter(
        (l) =>
          ...
          // "Pre Semi" / "Pre Quarter" / "Prefinals" name the round BEFORE a
          // stage, and which stage that is depends on the bracket size.
          // Guessing would overstate how far someone went, which is the
          // exact harm this vocabulary work exists to prevent — so these
          // stay unclassified on purpose.
          !/\bpre[-\s]?(semi|quarter|octo|final)/i.test(l),
      );
      const bad: string[] = [];
